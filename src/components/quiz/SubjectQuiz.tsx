...
export function SubjectQuiz({ subject, onComplete }: SubjectQuizProps) {
  const { user } = useAuth()
  ...
  const fetchSubjectQuestions = async () => {
    try {
      // Map subject IDs to database subject names
      ...
      // If no questions found for specific subject, get random questions
      if (!data || data.length === 0) {
        ...
        const transformedQuestions = (randomData || []).map(q => ({
          ...q,
      ...
      <Card className="max-w-2xl mx-auto">
        <CardContent className="flex items-center justify-center h-64">
          <div className="text-center space-y-4">
            <div className="text-lg">No questions available for this subject</div>
    ...
    return (
      <Card className="max-w-2xl mx-auto">
        <CardHeader className="text-center">
          ...
          <CardDescription>Great job on completing the subject test</CardDescription>
        </CardHeader>
